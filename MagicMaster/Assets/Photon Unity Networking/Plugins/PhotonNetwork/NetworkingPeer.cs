...
#region Operations and Connection Methods
    ...
    protected internal void SetupProtocol(ServerConnection serverType)
    {
        ...
        if (PhotonPeer.NoSocket)
        {
            ...
            {
                UnityEngine.Debug.Log("This Photon3Unity3d.dll only allows UDP. TransportProtocol was: " + this.TransportProtocol + ". SocketImplementation: " + this.SocketImplementation);
            }
            ...
            {
                UnityEngine.Debug.Log("No socket implementation set for 'NoSocket' assembly. Please check your settings.");
            }
        }
        ...
        if (PhotonNetwork.logLevel >= PhotonLogLevel.ErrorsOnly)
        {
            UnityEngine.Debug.LogWarning("Protocol switch from: " + this.TransportProtocol + " to: " + protocolOverride + ".");
        }
    ...
    private void HandleEventLeave(int actorID, EventData evLeave)
    {
        ...
        if (evLeave.Parameters.ContainsKey(ParameterCode.IsInactive))
        {
            ...
            {
                UnityEngine.Debug.LogWarning("HandleEventLeave for player ID: " + actorID + " isInactive: " + player.IsInactive + ". Stopping handling if inactive.");
                return;
            }
        }
    ...
    public void OnOperationResponse(OperationResponse operationResponse)
    {
        ...
        else
        {
            ...
            {
                UnityEngine.Debug.LogError("Operation failed: " + operationResponse.ToStringFull() + " Server: " + this.Server);
            }
        }
        ...
        switch (operationResponse.OperationCode)
        {
            ...
            case OperationCode.AuthenticateOnce:
            {
                ...
                if (operationResponse.ReturnCode != 0)
                {
                    ...
                    {
                        UnityEngine.Debug.LogError(string.Format("If you host Photon yourself, make sure to start the 'Instance LoadBalancing' "+ this.ServerAddress));
                    }
                    else if (operationResponse.ReturnCode == ErrorCode.InvalidAuthentication)
                    {
                        UnityEngine.Debug.LogError(string.Format("The appId this client sent is unknown on the server (Cloud). Check settings. If using the Cloud, check account."));
                    ...
                    {
                        UnityEngine.Debug.LogError(string.Format("Custom Authentication failed (either due to user-input or configuration or AuthParameter string format). Calling: OnCustomAuthenticationFailed()"));
                    ...
                    {
                        if (PhotonNetwork.logLevel >= PhotonLogLevel.Informational)
                            UnityEngine.Debug.LogWarning(string.Format("Currently, the limit of users is reached for this title. Try again later. Disconnecting"));
                    ...
                    {
                        if (PhotonNetwork.logLevel >= PhotonLogLevel.Informational)
							UnityEngine.Debug.Log(string.Format("The used master server address is not available with the subscription currently used. Got to Photon Cloud Dashboard or change URL. Disconnecting."));
                    ...
                    {
                        if (PhotonNetwork.logLevel >= PhotonLogLevel.Informational)
							UnityEngine.Debug.Log(string.Format("The authentication ticket expired. You need to connect (and authenticate) again. Disconnecting."));
            ...
            case OperationCode.GetRegions:
                ...
                {
				UnityEngine.Debug.Log(string.Format("The appId this client sent is unknown on the server (Cloud). Check settings. If using the Cloud, check account."));
                    SendMonoMessage(PhotonNetworkingMessage.OnFailedToConnectToPhoton, DisconnectCause.InvalidAuthentication);
                ...
                {
				UnityEngine.Debug.Log("GetRegions failed. Can't provide regions list. Error: " + operationResponse.ReturnCode + ": " + operationResponse.DebugMessage);
                    break;
                }
                ...
                {
				UnityEngine.Debug.Log("The region arrays from Name Server are not ok. Must be non-null and same length. " + (regions ==null)+ " " + (servers==null) + "\n"+operationResponse.ToStringFull());
                    break;
                }
                ...
                for (int i = 0; i < regions.Length; i++)
                {
                    ...
                    if (PhotonNetwork.PhotonServerSettings.HostType == ServerSettings.HostingOption.BestRegion && PhotonNetwork.PhotonServerSettings.EnabledRegions != 0)
                    {
                        ...
                        {
						UnityEngine.Debug.Log("Skipping region because it's not in PhotonServerSettings.EnabledRegions: " + code);
                        }
                    }
            ...
            case OperationCode.FindFriends:
				...
				UnityEngine.Debug.Log("FindFriends failed to apply the result, as a required value wasn't provided or the friend list length differed from result.");
                }
    ...
    public void OnStatusChanged(StatusCode statusCode)
    {
        ...
        switch (statusCode)
        {
			...
			UnityEngine.Debug.Log("Encryption wasn't established: " + statusCode + ". Going to authenticate anyways.");
            ...
            case StatusCode.Exception:
                if (this.IsInitialConnect)
                {
				UnityEngine.Debug.Log("Exception while connecting to: " + this.ServerAddress + ". Check if the server is available.");
            ...
            case StatusCode.TimeoutDisconnect:
                if (this.IsInitialConnect)
                {
                    UnityEngine.Debug.LogWarning(statusCode + " while connecting to: " + this.ServerAddress + ". Check if the server is available.");
            ...
            case StatusCode.QueueIncomingUnreliableWarning:
                UnityEngine.Debug.Log(statusCode + ". This client buffers many incoming messages. This is OK temporarily. With lots of these warnings, check if you send too much or execute messages too slow. " + (PhotonNetwork.isMessageQueueRunning? "":"Your isMessageQueueRunning is false. This can cause the issue temporarily.") );
                break;
    ...
    public void OnEvent(EventData photonEvent)
    {
        ...
        if (photonEvent.Parameters.ContainsKey(ParameterCode.ActorNr))
        {
            ...
            //    {
            //        UnityEngine.Debug.LogWarning("Received event, but we do not have this actor:  " + actorNr);
            //    }
            //}
        }
        ...
        switch (photonEvent.Code)
        {
            case PunEvent.OwnershipRequest:
            {
                ...
                {
                    UnityEngine.Debug.LogWarning("Can't find PhotonView of incoming OwnershipRequest. ViewId not found: " + requestedViewId);
                    break;
                }

                if (PhotonNetwork.logLevel == PhotonLogLevel.Informational)
                    UnityEngine.Debug.Log("Ev OwnershipRequest " + requestedView.ownershipTransfer + ". ActorNr: " + actorNr + " takes from: " + currentOwner + ". Current owner: " + requestedView.ownerId + " isOwnerActive: " + requestedView.isOwnerActive + ". MasterClient: " + this.mMasterClientId + ". This client's player: " + PhotonNetwork.player.ToStringFull());
                ...
                switch (requestedView.ownershipTransfer)
                {
                    ...
                    case OwnershipOption.Takeover:
                        if (currentOwner == requestedView.ownerId || (currentOwner == 0 && requestedView.ownerId == this.mMasterClientId))
                        {
                            ...
                            if (PhotonNetwork.logLevel == PhotonLogLevel.Informational)
                            {
                                UnityEngine.Debug.LogWarning(requestedView + " ownership transfered to: "+ actorNr);
                            }
                        }
                        break;
            ...
            case PunEvent.OwnershipTransfer:
                {
                    ...
                    UnityEngine.Debug.Log("Ev OwnershipTransfer. ViewID " + transferViewToUserID[0] + " to: " + transferViewToUserID[1] + " Time: " + Environment.TickCount%1000);
            ...
            case PunEvent.CloseConnection:
                ...
                {
                    UnityEngine.Debug.LogError("Error: Someone else(" + originatingPlayer + ") then the masterserver requests a disconnect!");
                }
                else
                {
            ...
            case PunEvent.DestroyPlayer:
                ...
                {
				if (this.DebugOut >= DebugLevel.INFO) UnityEngine.Debug.Log("Ev DestroyAll! By PlayerId: " + actorNr);
            ...
            case PunEvent.Destroy:
                ...
                {
                    if (this.DebugOut >= DebugLevel.ERROR) UnityEngine.Debug.LogError("Ev Destroy Failed. Could not find PhotonView with instantiationId " + instantiationId + ". Sent by actorNr: " + actorNr);
                }
    ...
    private void SendVacantViewIds()
    {
        ...
        UnityEngine.Debug.Log("Sending vacant view IDs. Length: " + vacantViews.Count);
    ...
    protected internal void ExecuteRpc(Hashtable rpcData, PhotonPlayer sender)
    {
        ...
        {
            UnityEngine.Debug.LogError("Malformed RPC; this should never occur. Content: " + SupportClassPun.DictionaryToString(rpcData));
            return;
        }
        ...
        if (rpcData.ContainsKey((byte)5))
        {
            ...
            {
                UnityEngine.Debug.LogError("Could not find RPC with index: " + rpcIndex + ". Going to ignore! Check PhotonServerSettings.RpcList");
                return;
            }
            else
            {
        ...
        if (photonNetview == null)
        {
            ...
            if (owningPv)
            {
                UnityEngine.Debug.LogWarning("Received RPC \"" + inMethodName + "\" for viewID " + netViewID + " but this PhotonView does not exist! View was/is ours." + (ownerSent ? " Owner called." : " Remote called.") + " By: " + sender.ID);
            }
            else
            {
                UnityEngine.Debug.LogWarning("Received RPC \"" + inMethodName + "\" for viewID " + netViewID + " but this PhotonView does not exist! Was remote PV." + (ownerSent ? " Owner called." : " Remote called.") + " By: " + sender.ID + " Maybe GO was destroyed but RPC not cleaned up.");
            }
            return;
        }
        ...
        {
            UnityEngine.Debug.LogError("Received RPC \"" + inMethodName + "\" on viewID " + netViewID + " with a prefix of " + otherSidePrefix + ", our prefix is " + photonNetview.prefix + ". The RPC has been ignored.");
            return;
        }
        ...
        if (PhotonNetwork.logLevel >= PhotonLogLevel.Full)
            UnityEngine.Debug.Log("Received RPC: " + inMethodName);
        ...
        if (receivers != 1)
        {
            ...
            if (receivers == 0)
            {
                if (foundMethods == 0)
                {
                    UnityEngine.Debug.LogError("PhotonView with ID " + netViewID + " has no method \"" + inMethodName + "\" marked with the [PunRPC](C#) or @PunRPC(JS) property! Args: " + argsString);
                }
                else
                {
                    UnityEngine.Debug.LogError("PhotonView with ID " + netViewID + " has no method \"" + inMethodName + "\" that takes " + argTypes.Length + " argument(s): " + argsString);
                }
            }
            else
            {
                UnityEngine.Debug.LogError("PhotonView with ID " + netViewID + " has " + receivers + " methods \"" + inMethodName + "\" that takes " + argTypes.Length + " argument(s): " + argsString + ". Should be just one?");
            }
        }
    }
    ...
    internal GameObject DoInstantiate(Hashtable evData, PhotonPlayer photonPlayer, GameObject resourceGameObject)
    {
        ...
        else
        {
            ...
            if (resourceGameObject == null)
            {
                ...
                if (resourceGameObject == null)
                {
                    UnityEngine.Debug.LogError("PhotonNetwork error: Could not Instantiate the prefab [" + prefabName + "]. Please verify you have this gameobject in a Resources folder.");
                    return null;
                }
            }
    ...
    protected internal void RemoveInstantiatedGO(GameObject go, bool localOnly)
    {
        ...
        if (!localOnly)
        {
            if (!viewZero.isMine)
            {
                UnityEngine.Debug.LogError("Failed to 'network-remove' GameObject. Client is neither owner nor masterClient taking over for owner who left: " + viewZero);
                return;
            }
            ...
            {
                UnityEngine.Debug.LogError("Failed to 'network-remove' GameObject because it is missing a valid InstantiationId on view: " + viewZero + ". Not Destroying GameObject or PhotonViews!");
                return;
            }
        }
        ...
        if (result == null)
        {
            ...
            for (int i = 0; i < views.Length; i++)
            {
                ...
                if (view.viewID == viewID)
                {
                    if (view.didAwake)
                    {
                        UnityEngine.Debug.LogWarning("Had to lookup view that wasn't in photonViewList: " + view);
                    }
                    return view;
                }
            }
    ...
    public void RegisterPhotonView(PhotonView netView)
    {
        ...
        if (isViewListed)
        {
            ...
            {
                UnityEngine.Debug.LogError(string.Format("PhotonView ID duplicate found: {0}. New: {1} old: {2}. Maybe one wasn't destroyed on scene load?! Check for 'DontDestroyOnLoad'. Destroying old entry, adding new.", netView.viewID, netView, listedView));
            }
            else
            {
                return;
        ...
        //UnityEngine.UnityEngine.Debug.LogError("view being added. " + netView);	// Exit Games internal log
    ...
    //    //{
    //    //    UnityEngine.Debug.LogError("Detected two differing PhotonViews with same viewID: " + netView.viewID);
    //    //}
    ...
    public void SetReceivingEnabled(int[] enableGroups, int[] disableGroups)
    {
        ...
        if (enableGroups != null)
        {
            for (int index = 0; index < enableGroups.Length; index++)
            {
                ...
                if (i <= 0)
                {
                    UnityEngine.Debug.LogError("Error: PhotonNetwork.SetReceivingEnabled was called with an illegal group number: " + i + ". The group number should be at least 1.");
                    continue;
                }
        ...
        if (disableGroups != null)
        {
            for (int index = 0; index < disableGroups.Length; index++)
            {
                ...
                {
                    UnityEngine.Debug.LogError("Error: PhotonNetwork.SetReceivingEnabled disableGroups contains a group that is also in the enableGroups: " + i + ".");
                    continue;
                }
                if (this.allowedReceivingGroups.Contains(i))
                {
    ...
    public void NewSceneLoaded()
    {
        ...
        {
            if (PhotonNetwork.logLevel >= PhotonLogLevel.Informational)
                UnityEngine.Debug.Log("New level loaded. Removed " + removeKeys.Count + " scene view IDs from last level.");
        }
    }
    ...
    private void OnSerializeRead(object[] data, PhotonPlayer sender, int networkTime, short correctPrefix)
    {
        ...
        if (view == null)
        {
            UnityEngine.Debug.LogWarning("Received OnSerialization for view ID " + viewID + ". We have no such PhotonView! Ignored this if you're leaving a room. State: " + this.State);
            return;
        }

        if (view.prefix > 0 && correctPrefix != view.prefix)
        {
            UnityEngine.Debug.LogError("Received OnSerialization for view ID " + viewID + " with prefix " + correctPrefix + ". Our prefix is " + view.prefix);
            return;
        }
        ...
        if (view.synchronization == ViewSynchronization.ReliableDeltaCompressed)
        {
            ...
            if (uncompressed == null)
            {
                ...
                if (PhotonNetwork.logLevel >= PhotonLogLevel.Informational)
                {
                    UnityEngine.Debug.Log("Skipping packet for " + view.name + " [" + view.viewID + "] as we haven't received a full packet for delta compression yet. This is OK if it happens for the first few frames after joining a game.");
                }
                return;
            }
    ...
    protected internal void SetLevelInPropsIfSynced(object levelId)
    {
        ...
        else UnityEngine.Debug.LogError("Parameter levelId must be int or string!");
