...
public static class PhotonNetwork
{
    ...
    static PhotonNetwork()
    {
        ...
        {
            //UnityEngine.Debug.Log(string.Format("PhotonNetwork.ctor() Not playing {0} {1}", UnityEditor.EditorApplication.isPlaying, UnityEditor.EditorApplication.isPlayingOrWillChangePlaymode));
            return;
        }
        ...
        if (photonHandlers != null && photonHandlers.Length > 0)
        {
            ...
            {
                //UnityEngine.Debug.Log("Handler: " + photonHandler + " photonHandler.gameObject: " + photonHandler.gameObject);
        ...
        #if UNITY_XBOXONE
        UnityEngine.Debug.Log("UNITY_XBOXONE is defined: Using AuthMode 'AuthOnceWss' and EncryptionMode 'DatagramEncryption'.");
        if (!PhotonPeer.NativeDatagramEncrypt)
        {
            UnityEngine.Debug.LogError("XB1 builds need a Photon3Unity3d.dll which uses the native PhotonEncryptorPlugin. This dll does not!");
        }
    ...
    {
        // UnityEngine.Debug.Log("SwitchToProtocol: " + cp + " PhotonNetwork.connected: " + PhotonNetwork.connected);
    ...
    public static bool ConnectUsingSettings(string gameVersion)
    {
        ...
        if (PhotonServerSettings == null)
        {
            UnityEngine.Debug.LogError("Can't connect: Loading settings failed. ServerSettings asset must be in any 'Resources' folder as: " + serverSettingsAssetFile);
            return false;
        }
    ...
    public static bool ConnectToRegion(CloudRegionCode region, string gameVersion)
    {
        ...
        if (PhotonServerSettings == null)
        {
            UnityEngine.Debug.LogError("Can't connect: ServerSettings asset must be in any 'Resources' folder as: " + PhotonNetwork.serverSettingsAssetFile);
            return false;
        }
    ...
    /// { UnityEngine.Debug.Log("OnEventHandler"); }
    ///
    ...
    public static GameObject Instantiate(string prefabName, Vector3 position, Quaternion rotation, int group, object[] data)
    {
        ...
        {
            //UnityEngine.Debug.Log("Instantiate prefabName: " + prefabName + " player.ID: " + player.ID);
    ...
    public static GameObject InstantiateSceneObject(string prefabName, Vector3 position, Quaternion rotation, int group, object[] data)
    {
        if (!connected || (InstantiateInRoomOnly && !inRoom))
        {
            UnityEngine.Debug.LogError("Failed to InstantiateSceneObject prefab: " + prefabName + ". Client should be in a room. Current connectionStateDetailed: " + PhotonNetwork.connectionStateDetailed);
            return null;
        }
    ...
    public static bool SetMasterClient(PhotonPlayer masterClientPlayer)
    {
        ...
        {
            if (logLevel == PhotonLogLevel.Informational) UnityEngine.Debug.Log("Can not SetMasterClient(). Not in room or in offlineMode.");
            return false;
        }
    ...
    public static void CreateSettings()
    {
        ...
        {
            UnityEngine.Debug.LogError("missing settings script");
            return;
        }
        ...
        if (PhotonNetwork.PhotonServerSettings == null)
        {
            ...
            {
                UnityEngine.Debug.LogError("PUN failed creating a settings file. ScriptableObject.CreateInstance(\"ServerSettings\") returned null. Will try again later.");
            }
        }
    }
    ...
    public static void InternalCleanPhotonMonoFromSceneIfStuck()
    {
        ...
        if (photonHandlers != null && photonHandlers.Length > 0)
        {
            ...
            foreach (PhotonHandler photonHandler in photonHandlers)
            {
                // UnityEngine.Debug.Log("Removing Handler: " + photonHandler + " photonHandler.gameObject: " + photonHandler.gameObject);
                photonHandler.gameObject.hideFlags = 0;
