...
public class PhotonViewHandler : EditorWindow
{
    ...
    internal static void HierarchyChange()
    {
        ...
        {
            //UnityEngine.Debug.Log("HierarchyChange ignored, while running.");
        ...
        //UnityEngine.Debug.Log("HierarchyChange. PV Count: " + pvObjects.Length);
        ...
        //UnityEngine.Debug.Log("Level '" + Application.loadedLevelName + "' has a minimum ViewId of: " + minViewIdInThisScene);
        ...
        foreach (PhotonView view in pvInstances)
        {
            if (view.viewID == 0)
            {
                ...
                // UnityEngine.Debug.LogWarning("setting scene ID: " + view.gameObject.name + " ID: " + view.subId.ID + " scene ID: " + view.GetSceneID() + " IsPersistent: " + EditorUtility.IsPersistent(view.gameObject) + " IsSceneViewIDFree: " + IsSceneViewIDFree(view.subId.ID, view));
        ...
        if (fixedSomeId)
        {
            //UnityEngine.Debug.LogWarning("Some subId was adjusted."); // this log is only interesting for Exit Games
        }
    }
