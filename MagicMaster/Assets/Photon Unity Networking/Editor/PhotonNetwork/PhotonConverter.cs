...
public class PhotonConverter : Photon.MonoBehaviour
{
        ...
        foreach (string prefab in prefabs)
        {
            ...
            if (movePrefabs && converted > 0)
            {
                ...
                if (resourcesIndex != lastSlash - 10)
                {
                    ...
                    {
                        UnityEngine.Debug.LogWarning("Warning, prefab [" + prefab + "] was already in a resources folder. But has been placed in the root of another one!");
                    }
    ...
    public static List<string> GetScriptsInFolder(string folder)
    {
        ...
        {
            UnityEngine.Debug.Log("Getting script list from folder " + folder + " failed. Exception:\n" + ex.ToString());
        }
        ...
        foreach (string script in scriptPathList)
        {
            if (script.Contains("PhotonNetwork")) //Don't convert this file (and others)
            {
                if (!ignoreWarningIsLogged)
                {
                    ...
                    UnityEngine.Debug.LogWarning("Conversion to PUN ignores all files with \"PhotonNetwork\" in their file-path.\nCheck: " + script);
                }
                continue;
            }
    ...
    public static void ConvertRpcAttribute(string path)
    {
        ...
        foreach (string file in scripts)
        {
            ...
            {
                File.WriteAllText(file, text);
                UnityEngine.Debug.Log("Converted RPC to PunRPC in: " + file);
            }
        }
    }
    ...
    static string PregReplace(string input, string[] pattern, string[] replacements)
    {
        if (replacements.Length != pattern.Length)
            UnityEngine.Debug.LogError("Replacement and Pattern Arrays must be balanced");
    ...
    {
        UnityEngine.Debug.Log(((int)EditorApplication.timeSinceStartup) + " " + str);
    }

    static void ConversionError(string file, string str)
    {
        UnityEngine.Debug.LogError("Scrip conversion[" + file + "]: " + str);
    }
