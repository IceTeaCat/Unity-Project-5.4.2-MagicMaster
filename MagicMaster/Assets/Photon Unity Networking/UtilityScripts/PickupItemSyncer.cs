...
public class PickupItemSyncer : Photon.MonoBehaviour
{
    ...
    public void AskForPickupItemSpawnTimes()
    {
        if (this.IsWaitingForPickupInit)
        {
            ...
            if (nextPlayer == null || nextPlayer.Equals(PhotonNetwork.player))
            {
                ...
                //UnityEngine.Debug.Log("This player is the Master's next. Asking this client's 'next' player: " + ((nextPlayer != null) ? nextPlayer.ToStringFull() : ""));
            }
    ...
    private void SendPickedUpItems(PhotonPlayer targetPlayer)
    {
        ...
        for (int i = 0; i < items.Length; i++)
        {
            ...
            else
            {
                ...
                if (pi.TimeOfRespawn > soon)
                {
                    ...
                    UnityEngine.Debug.Log(pi.ViewID + " respawn: " + pi.TimeOfRespawn + " timeUntilRespawn: " + timeUntilRespawn + " (now: " + PhotonNetwork.time + ")");
                    valuesToSend.Add(pi.ViewID);
        ...
        UnityEngine.Debug.Log("Sent count: " + valuesToSend.Count + " now: " + now);
    ...
    public void PickupItemInit(double timeBase, float[] inactivePickupsAndTimes)
    {
        ...
        for (int i = 0; i < inactivePickupsAndTimes.Length / 2; i++)
        {
            ...
            else
            {
                ...
                UnityEngine.Debug.Log(view.viewID + " respawn: " + timeOfRespawn + " timeUntilRespawnBasedOnTimeBase:" + timeUntilRespawnBasedOnTimeBase + " SecondsBeforeRespawn: " + pi.SecondsBeforeRespawn);
